...
│ src/retrieval.py         │ ⚠️ Needs Product Header Ext │ Extract product from Markdown '# ' line │
...
```
PHASE 1: Foundation Layer & Data Services (CURRENT)
...
- [x] `src/config.py`: Verified with `default_factory` and `validate_paths()`.
- [x] `src/schemas.py`: Verified with `extra="forbid"`, flexible `TicketInput`, and strict 3-section `AccountHealthBrief`.
- [x] `src/exceptions.py`: Verified with typed error hierarchy (`AccountNotFoundError`, `InvalidTicketError`, etc.).
- [x] `src/data_loader.py`: Verified with dynamic 90-day window anchor: $\text{reference\_date} = \max(\text{ticket.created\_at})$.
- [x] `src/normalization.py`: Verified with targeted PII masking, whitespace cleaning, and safe truncation.
- [x] `src/retrieval.py`: Verified with in-memory TF-IDF + exact regex error boost passing 4/4 pytest tests.
